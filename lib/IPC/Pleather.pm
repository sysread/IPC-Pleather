...
=head1 DIFFERENCES FROM CILK

  * Pleather's scheduler is very simple, using a semaphore to restrict the
    number of forks across the spawn tree
  * No support for inlets
  * Expands to Perl at compile time using pluggable keywords (and therefore
    requires 5.12 or later)
